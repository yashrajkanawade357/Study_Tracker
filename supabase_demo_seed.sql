-- ============================================================
-- Vyora — DEMO ACCOUNT seed data (for hackathon judges)
--
-- Creates a rich, "lived-in" dataset for a demo user so judges
-- can log in and explore every feature with realistic content.
--
-- The demo account is a NORMAL user (is_admin = false), so it can
-- see all the apps but NOT the admin portal — that stays with you.
--
-- PREREQUISITE: first create the auth user in the dashboard
--   Authentication > Users > Add user
--     email:    demo@vyora.app      (or change DEMO_EMAIL below)
--     password: (your choice — this is what you share with judges)
--     ☑ Auto Confirm User
--
-- Then paste this whole file into the SQL Editor and Run.
-- Safe to re-run: it wipes the demo user's data and reseeds.
-- ============================================================

do $$
declare
  demo_email text := 'demo@vyora.app';   -- <<< must match the email you created
  uid uuid;
...
begin
  ...
  if uid is null then
    raise exception 'No auth user with email %.  Create it in Authentication > Users first.', demo_email;
  end if;

  -- ---- wipe any previous demo data (idempotent) -----------------
  ...
  -- ---- profile (non-admin; skip onboarding + manual gates) ------
  ...
  -- ---- study logs: ~75 days of activity for charts/heatmap -----
  ...
  from (
    ...
    where s = 1 or random() > 0.4          -- ~60% of days get a 2nd session
  ) g;

  -- A few "special" sessions so the matching achievements make sense.
  ...
  -- ---- achievements (a believable unlocked set) ----------------
  ...
  -- ---- calendar events (next week; reminders OFF for demo) ------
  ...
  -- ---- tasks (mix of pending + done; reminders OFF) ------------
  ...
  raise notice 'Demo data seeded for % (user_id %).', demo_email, uid;
end $$;
